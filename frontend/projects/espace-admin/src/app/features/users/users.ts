...
export class UsersComponent {
  ...
  this.http.get<any>(`${this.api}/gestion`, { headers }).subscribe({
    next: (res) => {
      setTimeout(() => {
  ...
  saveUser() {
const token = sessionStorage.getItem('stb_token');
  const headers = { Authorization: `Bearer ${token}` };
  ...
  if (this.isEditing && this.editingIndex !== -1) {
    ...
    this.http.put(`${this.api}/${id}`, payload, { headers }).subscribe({
      next: () => {
        this.loadUsers();
    ...
    this.http.post(this.api, payload, { headers }).subscribe({
      next: () => {
        this.loadUsers();
...
suspendre(u: any) {
 const token = sessionStorage.getItem('stb_token');
  ...
  this.http.put(`${this.api}/${u.id}/statut`, {}, { headers }).subscribe({
    next: () => {
      this.loadUsers();
      this.toastService.show(`Statut de ${u.nom} mis à jour`, 'error');
    },
