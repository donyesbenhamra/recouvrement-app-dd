    ...
    public class IntentionController : ControllerBase
    {
            ...
            try
            {
                ...
                var items = intentionsList.Select(i => 
                {
                    ...
                    return new IntentionItemDto
                    {
                     ...
                     ConfianceClient = i.ConfianceClient ?? 0,
        ...
        public async Task<IActionResult> MakeDecision(int id, [FromBody] IntentionDecisionDto decisionDto)
        {
            try
            {
                ...
                string messageClient = decisionDto.Decision == "Accepter"
             ? $"✅ Votre demande de '{intention.TypeIntention.Replace("_", " ")}' a été acceptée par votre agence. Nous vous contacterons prochainement."
            : $"❌ Votre demande de '{intention.TypeIntention.Replace("_", " ")}' a été refusée. Veuillez contacter votre agence pour plus d'informations.";
